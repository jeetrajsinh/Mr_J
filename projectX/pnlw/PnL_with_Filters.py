...
min_winrate = float(input("Enter minimum win rate (default 40%): ") or 40.0)
max_winrate = float(input("Enter maximum win rate (default 90%): ") or 90.0)
min_roi = float(input("Enter minimum ROI (default 10%): ") or 10.0)
max_roi = float(input("Enter maximum ROI (default 500%): ") or 500.0)
min_sol_balance = float(input("Enter minimum SOL balance (default 2): ") or 2.0)
max_sol_balance = float(input("Enter maximum SOL balance (default 10000): ") or 10000.0)
min_txns = int(input("Enter minimum transactions (default 50): ") or 50)
max_txns = int(input("Enter maximum transactions (default 1000): ") or 1000)
min_pnl_2x_5x = int(input("Enter minimum PnL 2x-5x (default 0): ") or 0)
max_pnl_2x_5x = int(input("Enter maximum PnL 2x-5x (default 1000): ") or 1000)

print(f"[🤖] Filtering criteria set:")
print(f" - Win Rate: {min_winrate}% to {max_winrate}%")
print(f" - ROI: {min_roi}% to {max_roi}%")
print(f" - SOL Balance: {min_sol_balance} to {max_sol_balance}")
print(f" - Transactions: {min_txns} to {max_txns}")
print(f" - PnL 2x-5x: {min_pnl_2x_5x} to {max_pnl_2x_5x}")
...
# Save wallet data to Excel with filtering
def save_to_excel(wallet_data):
    ...
    for wallet, winrate, roi, sol_balance, transactions, token_avg_cost, pnl_lt_2x, pnl_2x_5x, pnl_gt_5x, last_active in wallet_data:
        if (min_winrate <= winrate <= max_winrate and
            min_roi <= roi <= max_roi and
            min_sol_balance <= sol_balance <= max_sol_balance and
            min_txns <= transactions <= max_txns and
            min_pnl_2x_5x <= pnl_2x_5x <= max_pnl_2x_5x):
